package ci.himaf.civ_idcard_scanner.scanner_core.scan_filters

import android.util.Log
import com.google.mlkit.vision.text.Text
import ci.himaf.civ_idcard_scanner.logger.debugLog
import ci.himaf.civ_idcard_scanner.scanner_core.constants.CardHolderNameConstants
import ci.himaf.civ_idcard_scanner.scanner_core.constants.CardScannerRegexps
import ci.himaf.civ_idcard_scanner.scanner_core.models.*
import java.util.*
import kotlin.math.max
import kotlin.math.min

class CardFirstNameFilter(visionText: Text, scannerOptions: CardScannerOptions, private val cardNumberScanResult: CardNumberScanResult) : ScanFilter(visionText, scannerOptions) {
  private val cardHolderRegex: Regex = Regex(CardScannerRegexps.cardFirstName, RegexOption.MULTILINE)
  val _maxBlocksBelowCardNumberToSearchForName = 2;

  override fun filter(): CardFirstNameScanResult? {
    ...
    for (index in minTextBlockIndexToSearchName..maxTextBlockIndexToSearchName) {
      ...
      if (isValidName(cardHolderName)) {
        return CardFirstNameScanResult(
                textBlockIndex = index, textBlock = block, cardFirstName = cardHolderName, visionText = visionText);
      }
    }
    return null;
  }
