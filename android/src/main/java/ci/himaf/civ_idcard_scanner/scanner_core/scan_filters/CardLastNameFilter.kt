...
class CardLastNameFilter(visionText: Text, scannerOptions: CardScannerOptions, private val cardNumberScanResult: CardNumberScanResult) : ScanFilter(visionText, scannerOptions) {
  private val cardHolderRegex: Regex = Regex(CardScannerRegexps.cardLastName, RegexOption.MULTILINE)
  val _maxBlocksBelowCardNumberToSearchForName = 4;

  override fun filter(): CardLastNameScanResult? {
    ...
    for (index in minTextBlockIndexToSearchName..maxTextBlockIndexToSearchName) {
      ...
      if (isValidName(cardHolderName)) {
        return CardLastNameScanResult(
                textBlockIndex = index, textBlock = block, cardLastName = cardHolderName, visionText = visionText);
      }
    }
    return null;
  }
